    ...
    <div class="space-y-6">
      <div class="card">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
          <label class="field-label">样品</label><input pInputText [(ngModel)]="filter.kw" placeholder="搜索结果/样品/编号" class="w-full" (ngModelChange)="applyFilter()">
        </div>
          <div>
          ...
          <p-table [value]="filtered()" [paginator]="true" [rows]="15" [rowsPerPageOptions]="[15, 30, 60" size="small" responsiveLayout="scroll" [tableStyle]="{'min-width':'85rem'}">
            ...
            <ng-template pTemplate="body" let-r>
              <tr [style]="{ background: r.is_oos ? '#fef2f2' : r.is_oot ? '#fffbeb' : '' }">
                ...
                <td class="text-xs text-gray-600">{{ r.method_name || '' }} / {{ r.instrument || '' }}</td>
                <td>
                  <span class="badge" [ngClass]="statusClass(r.status)">{{ statusLabel(r.status) }}
                </td>
                <td>
                ...
                <td class="text-xs">{{ r.approved_by_name || (r.status === 'approved' ? '已批准' : '-' }}
                </td>
            ...
            <ng-template pTemplate="emptymessage"><tr><td colspan="9"><div class="empty-state"><i class="pi pi-list-check"></i><h3>暂无检测结果</h3><p>从「录入检测</div></td></tr></ng-template>
          </p-table>
        }
      </div>
    </div>
